resource "aws_cloudwatch_log_group" "red_log_group" {
  name              = "/aws/ecs/service/${var.service_name}"
  retention_in_days = 7
}


resource "aws_security_group" "red_security_group" {
  ...
  ingress {
    ...
    to_port         = 15000
    protocol        = "tcp"
    security_groups = [var.app_mesh_green_sg]
  }
...
data "aws_iam_policy_document" "red_task_init_iam_policy" {
  statement {
    actions = [
      "s3:GetObject",
      "s3:ListBucket"
...
data "aws_iam_policy_document" "red_task_firelens_iam_policy" {
  statement {
    actions = [
...
resource "aws_iam_role" "red_task_iam_role" {
  name_prefix = var.service_name
  ...
  inline_policy {
    ...
    policy = data.aws_iam_policy_document.red_task_init_iam_policy.json
  }

  inline_policy {
    name   = "firelens"
    policy = data.aws_iam_policy_document.red_task_firelens_iam_policy.json
  }
...
resource "aws_appmesh_virtual_node" "red_virtual_node" {
...
module "container_envoy" {
  ...
  app_mesh_virtual_node_arn = aws_appmesh_virtual_node.red_virtual_node.arn
  log_group                 = aws_cloudwatch_log_group.red_log_group.name
...
module "container_web_app" {
  ...
  log_configuration = {
    logDriver = "awslogs"
    options = {
      awslogs-group         = "${aws_cloudwatch_log_group.red_log_group.id}"
...
resource "aws_ecs_task_definition" "red_task_definition" {
  ...
  task_role_arn            = aws_iam_role.red_task_iam_role.arn
  cpu                      = 256
  memory                   = 512
  ...
  depends_on = [
    module.container_init,
    module.container_envoy,
    aws_service_discovery_service.red_cloud_map_service,
  ]
...
resource "aws_service_discovery_service" "red_cloud_map_service" {
  name         = var.service_name
...
resource "aws_ecs_service" "red_service" {
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.red_cloud_map_service.arn
  }
  ...
  network_configuration {
    ...
    security_groups = [aws_security_group.red_security_group.id]
  }

  depends_on = [
    aws_ecs_task_definition.red_task_definition
  ]
}

resource "aws_appmesh_virtual_router" "red_virtual_router" {
