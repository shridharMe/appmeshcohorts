resource "aws_cloudwatch_log_group" "orange_log_group" {
  name              = "/aws/ecs/service/${var.service_name}"
  retention_in_days = 7
}


resource "aws_security_group" "orange_security_group" {
...
data "aws_iam_policy_document" "orange_task_init_iam_policy" {
  statement {
    actions = [
      "s3:GetObject",
      "s3:ListBucket"
...
data "aws_iam_policy_document" "orange_task_firelens_iam_policy" {
  statement {
    actions = [
...
resource "aws_iam_role" "orange_task_iam_role" {
  name_prefix = var.service_name
  ...
  inline_policy {
    ...
    policy = data.aws_iam_policy_document.orange_task_init_iam_policy.json
  }

  inline_policy {
    name   = "firelens"
    policy = data.aws_iam_policy_document.orange_task_firelens_iam_policy.json
  }
...
resource "aws_appmesh_virtual_node" "orange_virtual_node" {
...
module "container_init" {
  ...
  log_group  = aws_cloudwatch_log_group.orange_log_group.name
...
module "container_envoy" {
  ...
  app_mesh_virtual_node_arn = aws_appmesh_virtual_node.orange_virtual_node.arn
  log_group                 = aws_cloudwatch_log_group.orange_log_group.name
...
module "container_web_app" {
  ...
  log_configuration = {
    logDriver = "awslogs"
    options = {
      awslogs-group         = "${aws_cloudwatch_log_group.orange_log_group.id}"
...
resource "aws_ecs_task_definition" "orange_task_definition" {
  family                   = var.service_name
  network_mode             = "awsvpc"
  requires_compatibilities = ["FARGATE"]
  execution_role_arn       = var.task_execution_role
  task_role_arn            = aws_iam_role.orange_task_iam_role.arn
  cpu                      = 256
  memory                   = 512
  ...
  depends_on = [
    module.container_init,
    module.container_envoy,
    aws_service_discovery_service.orange_cloud_map_service,
  ]
...
resource "aws_service_discovery_service" "orange_cloud_map_service" {
  name         = var.service_name
...
resource "aws_ecs_service" "orange_service" {
  name            = var.service_name
  cluster         = var.ecs_cluster
  task_definition = aws_ecs_task_definition.orange_task_definition.arn
  desired_count   = 1
  ...
  service_registries {
    registry_arn = aws_service_discovery_service.orange_cloud_map_service.arn
  }
  ...
  network_configuration {
    ...
    security_groups = [aws_security_group.orange_security_group.id]
  }

  depends_on = [
    aws_ecs_task_definition.orange_task_definition
  ]
}
...
resource "aws_appmesh_virtual_router" "orange_virtual_router" {
...
resource "aws_appmesh_route" "orange_virtual_route" {
  name                = "${var.service_name}-color-route"
  mesh_name           = var.app_mesh_mesh_name
  virtual_router_name = aws_appmesh_virtual_router.orange_virtual_router.name
  ...
  spec {
    http_route {
      ...
      action {
        weighted_target {
          virtual_node = aws_appmesh_virtual_node.orange_virtual_node.name
          weight       = 100
        }
      }
    }
...
resource "aws_appmesh_virtual_service" "orange_virtual_service" {
...
resource "aws_appmesh_gateway_route" "orange_gateway_route" {
  name                 = "orange"
  mesh_name            = var.app_mesh_mesh_name
  ...
  spec {
    http_route {
      action {
        target {
          virtual_service {
            virtual_service_name = aws_appmesh_virtual_service.orange_virtual_service.name
          }
        }
      }
